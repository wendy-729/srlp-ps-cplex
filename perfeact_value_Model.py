...
# 第几个实例98
# 9 is solved
# 9	46.0	20178.8310	2	100	100	0
    for instance in range(2, 3):
        for dtime in dtimes:
            # 获得可行解的个数
            feasible = 0
            # 写入实验结果文件
            filename = r'C:\Users\ASUS\Desktop\model实验结果\J' + str(actNumber)+ '\\' + 'model_perfect' + '_dtime_' + str(dtime) + '.txt'
            with open(filename, 'a', newline='') as f:
                ...
                for mean_scenarios in mean_scenarios_Set:
                    ...
                    for scen in range(mean_scenarios):
                        ...
                        for kk in k:
                            for t in range(1, lftn + 1):
                                md1.add_constraint(
                                    ...
                                    #     range(max(est[i], t - duration[i]), min(t - 1, lst[i]) + 1))) <= z_kt[kk, t]
                                )
                        ...
                        # md1.parameters.mip.tolerance.mipgap =
                        # md1.parameters.threads = 1

                        if solution == None:
                            print('求不出解')
                            break  #只要有一个情景没有求出最优解，则不继续再求，转而求下一个实例
                            # continue
                        else:
                            # 记录几个情景
                            count += 1
                            ...
                            # print(solution)
                            # dd = solution.get_objective_value()
                            # print(dd)
                            print(d1)
                            # 获取执行活动和开始时间
                    ...
                    results =''
                    if count == mean_scenarios or count >= mean_scenarios * 1:
                        mean_perfect_value = mean_value/count
                        mean_cputime = all_cputime
                        results = str(instance) + '\t' + str(mean_perfect_value) + '\t' + format(mean_cputime, '.4f') + '\t' + \
                                  str(status.value) +'\t'+str(mean_scenarios) + '\t'+str(count)+ '\t'+str(feasible) +'\n'
                    else:
                        results = str(instance)+'\t'+str(0) +'\n'
