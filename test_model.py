        ...
        for dtime in dtimes:
            ...
            for nscen in scenariosSet:
                ...
                with open(filename, 'a', newline='') as f:
                    ...
                    x_itw = md1.binary_var_cube(act, d, w, name='x')
                    ...
                    # 辅助变量
                    z_ktw = md1.integer_var_cube(k, d, w, name='z')
                    ...
                    for s in range(scenarios):
                        md1.add_constraints(
                            md1.sum(x_itw[i, t, s] for t in list(range(scen_est[s][i], scen_lst[s][i] + 1))) == 1 for i in mandatory)

                    # 可选活动集合约束
                    for s in range(scenarios):
                        for ii in ae:
                            md1.add_constraint(
                                md1.sum(x_itw[i, t, s] for i in we[ae.index(ii)] for t in
                                        list(range(scen_est[s][i], scen_lst[s][i] + 1))) ==
                                md1.sum(x_itw[ii, tt, s] for tt in list(range(scen_est[s][ii], scen_lst[s][ii] + 1))))
                    ...
                    for s in range(scenarios):
                        for a in be:
                            for i in b[be.index(a)]:
                                md1.add_constraint(
                                    md1.sum(x_itw[i, t, s] for t in list(range(scen_est[s][i], scen_lst[s][i] + 1))) == \
                                    md1.sum(x_itw[a, tt, s] for tt in list(range(scen_est[s][a], scen_lst[s][a] + 1))))

                    # 截止日期约束
                    for i in range(scenarios):
                        md1.add_constraint(md1.sum(
                            t * x_itw[actNo - 1, t, i] for t in list(range(scen_est[i][actNo - 1], scen_lst[i][actNo - 1] + 1)
                                 )) - M * (1 - db_w[i]) <= lftn)
                            ...
                            for i in projPred[j]:
                                md1.add_constraint(
                                    md1.sum((t + duration[i]) * x_itw[i, t, s] for t in
                                            list(range(scen_est[s][i], scen_lst[s][i] + 1))) - M * (1 - pr_w[s]) \
                                    <= md1.sum(
                                        tt * x_itw[j, tt, s] for tt in list(range(scen_est[s][j], scen_lst[s][j] + 1))) + \
                                    M * (1 - md1.sum(
                                        x_itw[j, tt, s] for tt in list(range(scen_est[s][j], scen_lst[s][j] + 1)))))
                    ...
                    for s in range(scenarios):
                        ...
                        for kk in k:
                            for t in max_d:
                                md1.add_constraint(
                                    md1.sum(y_kthw[kk, t, h, s] for h in list(range(1, max_H + 1))) >= md1.sum(
                        ...
                        for t in max_d:
                            for kk in k:
                                md1.add_constraint(
                                    ...
                                    md1.sum(y_kthw[kk, t, h, s] for h in list(range(1, max_H + 1)))
                    ...
                    for s in w:
                        for kk in k:
                            md1.add_constraint(
                                z_ktw[kk, 0, s] == md1.sum(y_kthw[kk, 0, h, s] for h in list(range(1, max_H + 1))))
                    ...
                    if solution == None:
                        ...
                        # 将实验结果写入文件
                        results = str(instance) + '\t' +str(status.value) + '\t' + str(format(d1, '.4f')) + '\t' + str(format(cputime, '.4f') )+ '\t' + str(scenarios)+'\n'
                        print(instance,'is solved')
                        print(results)
                        # 写入文件
                        # f.write(results)
                        # # csv_results = [instance, d1, cputime, status.value]
                        #
                        #
                        ...
                        # print(scenarios_act_time)
                        # print(len(scenarios_act_time))
                        # 每个情景下的执行活动和开始时间
