        ...
        for dtime in dtimes:
            ...
            for nscen in scenariosSet:
                ...
                with open(filename, 'a', newline='') as f:
                    ...
                    # # 扩大的截止日期，读取DE仿真的gap
                    # gap_datafile = r'C:\Users\ASUS\Desktop\测试实验结果20211007\DE\J'+str(actNumber)+'\\'+'4000_sch_de_srlp_'+str(actNumber+2)+'_dt_'+str(dtime)+'_end_gap_200.txt'
                    ...
                    # 读取随机工期
                    if actNumber==5 or actNumber == 10:
                        file_duration = r'C:\Users\ASUS\Desktop\SRLP-PS随机工期\J' + str(actNumber) + '\\J' + str(actNumber) + '_' + str(
                    ...
                    db_w = md1.continuous_var_list(w, name='db')
                    # 优先关系是否满足的二元变量
                    # pr_w = md1.binary_var_list(w, name='pr')
                    pr_w = md1.continuous_var_list(w, name='pr')
                    # 资源约束是否满足的二元变量
                    # res_w = md1.binary_var_list(w, name='res')
                    res_w = md1.continuous_var_list(w, name='res')
                    ...
                    z_ktw = md1.continuous_var_cube(k, max_d, w, name='z')
                    ...
                    for s in range(scenarios):
                        ...
                        for kk in k:
                            for t in max_d:
                                md1.add_constraint(
                                    md1.sum(u_ktw[kk, t, s]) == md1.sum(req[i][kk] * x_itw[i, tt, s] for i in list(range(1, actNo)) for
